...
return {
    ...
    getUid: function(prefix){
        ...
        if(prefix){
            ...
            // 前缀最后一个字符如果是数字，添加用下划线和后面的数字id分隔
    ...
    browser: (function(){
        ...
        var data = {
            ...
            supportOrientation: 'orientation' in win, // 设备方向事件
            supportDeviceMotion: 'ondevicemotion' in win // 设备运动事件
        };

        //`localStorage` is null or `localStorage.setItem` throws error in some cases (e.g. localStorage is disabled)
        // localStorage为空或者存值报错在一些状况下（例如localStorage被禁止）
        try{
            var value = 'hilo';
        ...
        //css transform/3d feature dectection 功能检测
    ...
    getElementRect: function(elem){
        try{
            ...
            // bottom: Y轴，相对于视口原点（viewport origin）矩形盒子的底部。只读。 
            // height: 矩形盒子的高度（等同于 bottom 减 top）。只读。
            // left: X轴，相对于视口原点（viewport origin）矩形盒子的左侧。只读。 
            // right: X轴，相对于视口原点（viewport origin）矩形盒子的右侧。只读。 
            // top: Y轴，相对于视口原点（viewport origin）矩形盒子的顶部。只读。
